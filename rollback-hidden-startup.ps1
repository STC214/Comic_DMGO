...
$previous = Join-Path $root "artifacts\original\ComicPC.pre-hidden-startup-fix.exe"
...
if (!$Apply) { Write-Output "VERIFY_ONLY root=$root previous_sha256=$previousHash current_exists=$(Test-Path -LiteralPath $current)"; exit 0 }
